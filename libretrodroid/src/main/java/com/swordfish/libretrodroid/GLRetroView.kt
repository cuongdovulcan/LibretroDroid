...
class GLRetroView(
    ...
    suspend fun <T> runOnGLThread(
        ...
        val deferredResult: Deferred<T?> = getTaskNativeCode(block)
    ...
    private fun <T> getTaskNativeCode(block: suspend CoroutineScope.() -> T): Deferred<T> = GLScope.async {
        return@async if (Thread.currentThread().name.startsWith("GLThread")) {
            block()
        } else {

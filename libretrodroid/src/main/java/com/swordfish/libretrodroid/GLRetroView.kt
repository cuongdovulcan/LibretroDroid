...
import nativeOperationDispatcher
...
class GLRetroView(
    ...
    suspend fun <T> runOnGLThread(
        ...
        providedDispatcher: CoroutineDispatcher, // Use this passed-in dispatcher
        ...
        try {
            resultFromBlock = withTimeout(timeoutMillis) {
                if (Thread.currentThread().name.startsWith("GLThread")) { // GLSurfaceView's render thread
                    ...
                    withContext(providedDispatcher) {
